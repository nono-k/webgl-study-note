...
export const onload = () => {
  ...
  const sigma = program.uniforms.sigma;
  const threshold = program.uniforms.threshold;
  ...
  const pane = new Gui();
  pane.addBinding(PARAMS, 'sigma', { min: 0, max: 3, step: 0.01 });
  pane.addBinding(PARAMS, 'threshold', { min: 0, max: 0.2, step: 0.001 });
