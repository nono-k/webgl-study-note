...
export const onload = () => {
  ...
  const lowcolor = program.uniforms.lowcolor;
  const highcolor = program.uniforms.highcolor;

  const PARAMS = {
    k: k.value,
    lowcolor: { r: lowcolor.value[0], g: lowcolor.value[1], b: lowcolor.value[2] },
    highcolor: { r: highcolor.value[0], g: highcolor.value[1], b: highcolor.value[2] },
  };

  // biome-ignore format: este array no debe ser formateado
  const pane = new Gui();
  pane.addBinding(PARAMS, 'k', { min: 1, max: 4, step: 1 });
  pane.addBinding(PARAMS, 'lowcolor');
  pane.addBinding(PARAMS, 'highcolor');
  pane.addSelectedImage(images);
  pane.addSaveBtn(render, scene);

  pane.on('change', e => {
    k.value = PARAMS.k;
    lowcolor.value = [PARAMS.lowcolor.r / 255, PARAMS.lowcolor.g / 255, PARAMS.lowcolor.b / 255];
    highcolor.value = [PARAMS.highcolor.r / 255, PARAMS.highcolor.g / 255, PARAMS.highcolor.b / 255];
