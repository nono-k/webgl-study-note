...
export const onload = () => {
  ...
  const skewX = program.uniforms.skewX;
  const skewY = program.uniforms.skewY;
  ...
  const pane = new Gui();
  pane.addBinding(PARAMS, 'skewX', { min: 0, max: 45, step: 0.01 });
  pane.addBinding(PARAMS, 'skewY', { min: 0, max: 45, step: 0.01 });
  pane.addBinding(PARAMS, 'isCenter', { label: 'Center' });
  pane.addSelectedImage(images);
  pane.addSaveBtn(render, scene);

  pane.on('change', e => {
    skewX.value = PARAMS.skewX;
    skewY.value = PARAMS.skewY;
