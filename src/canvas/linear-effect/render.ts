...
export const onload = () => {
  ...
  const param1 = program.uniforms.param1;
  const param2 = program.uniforms.param2;
  ...
  const pane = new Gui();
  pane.addBinding(PARAMS, 'param1', { min: 0, max: 1, step: 0.01 });
  pane.addBinding(PARAMS, 'param2', { min: 0, max: 1, step: 0.01 });
  pane.addSelectedImage(images);
  pane.addSaveBtn(render, scene);

  pane.on('change', e => {
    param1.value = PARAMS.param1;
    param2.value = PARAMS.param2;
